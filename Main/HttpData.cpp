...
URIState HttpData::parseURI() {
  ...
  std::cout<<posGet<<"**"<<posPost<<"***"<<posHead<<"*******";
  ...
  std::cout<<pos;
  if (pos < 0) {
    fileName_ = "/root/MingMing/src/index.html";
    HTTPVersion_ = HTTP_11;
...
AnalysisState HttpData::analysisRequest() {
  if (method_ == METHOD_POST) {
    ...
    header += "Server: MingMing Web Server\r\n";
    // 头部结束
    header += "\r\n";
    outBuffer_ += header;
    // 打开请求的文件
    ...
    if (src_fd < 0) {
      // 如果文件打开失败
      outBuffer_.clear();// 清空输出缓冲区
      handleError(fd_, 404, "Not Found!");// 处理404错误
      return ANALYSIS_ERROR;// 返回解析错误
    }
    //// 将文件映射到内存中
    void *mmapRet = mmap(NULL, sbuf.st_size, PROT_READ, MAP_PRIVATE, src_fd, 0);
    close(src_fd);// 关闭文件描述符
    if (mmapRet == (void *)-1) {
      munmap(mmapRet, sbuf.st_size);// 如果映射失败
      outBuffer_.clear();
      handleError(fd_, 404, "Not Found!");
      return ANALYSIS_ERROR;
    }
    char *src_addr = static_cast<char *>(mmapRet);// 获取映射地址
    outBuffer_ += string(src_addr, src_addr + sbuf.st_size);// 将文件内容添加到输出缓冲区
    ;
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
  ...
  body_buff += "<hr><em> MingMing Web Server</em>\n</body></html>";

  header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
  header_buff += "Content-Type: text/html\r\n";
  header_buff += "Connection: Close\r\n";
  header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
  header_buff += "Server: MingMing Web Server\r\n";
  ;
  header_buff += "\r\n";
  // 错误处理不考虑writen不完的情况
